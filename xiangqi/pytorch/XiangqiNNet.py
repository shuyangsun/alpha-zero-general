...
class XiangqiNNet(nn.Module):
    ...
    def __init__(self, game, args):
        ...
        self.conv1 = nn.Conv1d(1, args.num_channels, 3, stride=1, padding=1)
        self.conv2 = nn.Conv1d(
            args.num_channels, args.num_channels, 3, stride=1, padding=1
        )
        self.conv3 = nn.Conv1d(args.num_channels, args.num_channels, 3, stride=1)
        self.conv4 = nn.Conv1d(args.num_channels, args.num_channels, 3, stride=1)
        ...
        self.fc1 = nn.Linear(args.num_channels * (self.board_size - 4), 1024)
    ...
    def forward(self, s):
        ...
        )  # batch_size x num_channels x (board_size-2)
        s = F.relu(
            self.bn4(self.conv4(s))
        )  # batch_size x num_channels x (board_size - 4)
        s = s.view(-1, self.args.num_channels * (self.board_size - 4))
